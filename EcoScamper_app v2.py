    ...
    if st.session_state.searched:
        ...
        if filtered_df.empty:
            ...
            for index, row in filtered_df.iterrows():
                ...
                # --- NEW: Logic for creating a clickable hyperlink for the case study ---
                case_name_bold = f"**{row['Case']}**"
                case_link = row.get('Link', '') # Safely get the link
                
                if pd.notna(case_link) and case_link.strip(): # Check if link exists and is not empty
                    case_display_text = f"[{case_name_bold}]({case_link})"
                else:
                    case_display_text = case_name_bold

                st.markdown(f"##### Category: **{row['Category']}** | Case Study: {case_display_text}", unsafe_allow_html=True)
